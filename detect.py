    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                #     n = (det[:, -1] == c).sum()  # detections per class. Number of hyperbola here
                #     with open(txt_path + '.txt', 'a') as f:
                #         f.write(f'Number of Hyperbola : {n}' + '\n')
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        # xywh.append(center_trace)   # Append the center of the bbox(possibly center trace of hyperbola) 74 = pixel no of white background on left.
                ...
                # Write min cover
                # with open(txt_path + '.txt', 'a') as f:
                #     f.write(f'Center Traces: {center_traces}' + '\n')
                #     f.write(f'Minimum cover trace: {min_trace}' + '\n')
...
def fromR(filename):
    check_requirements(exclude=('tensorboard', 'thop'))
    arguments = {
        'weights':['greyBest(edge Included).pt'],
        'imgsz':[480, 480], 
        'conf_thres':0.6, 
        'iou_thres':0.45, 
        'max_det':1000, 
